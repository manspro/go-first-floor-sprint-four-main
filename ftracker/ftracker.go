...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	caloriesBurned := (runningCaloriesMeanSpeedMultiplier * meanSpeed * runningCaloriesMeanSpeedShift * weight) * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speed := meanSpeed(action, duration) * kmhInMsec // преобразование скорости из км/ч в м/с
	caloriesBurned := (walkingCaloriesWeightMultiplier*weight +
		(math.Pow(speed, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) *
		duration * minInH
	return caloriesBurned
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	caloriesBurned := (meanSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
